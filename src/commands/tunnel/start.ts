  ...
  .action((opts) => {
    ...
    if (platform === "linux") {
      ...
      if (!fs.existsSync(servicePath)) {
        console.error(
          chalk.red(
            `The systemd service for tunnel "${tunnel}" does not exist. Did you run 'tunneler tunnel service install'?`,
          ),
        );
        process.exit(1);
      }
      ...
      if (!fs.existsSync(plistPath)) {
        console.error(
          chalk.red(
            `The LaunchAgent plist for tunnel "${tunnel}" does not exist. Did you run 'tunneler tunnel service install'?`,
          ),
        );
        process.exit(1);
      }
